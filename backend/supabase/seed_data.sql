...
INSERT INTO public.products (name, slug, price, category_id, image_url, is_featured)
SELECT
  p.name, p.slug, p.price, c.id, p.image_url, p.is_featured
FROM (
  VALUES
...
) AS p(name, slug, price, category_name, image_url, is_featured)
LEFT JOIN public.categories c ON c.name = p.category_name
