...
const TryOnCanvas = ({ userImage, glassesImage, size, rotation, opacity = 1, canvasRef }) => {
    ...
    const [userImageLoaded, setUserImageLoaded] = useState(0);
    ...
    // Load user image — use state counter to trigger redraw via the main draw effect  
    useEffect(() => {
        userImgRef.current = null;
        if (!userImage) { setUserImageLoaded(0); return; }
        const img = new Image();
        img.src = userImage;
        img.onload = () => {
            userImgRef.current = img;
            setUserImageLoaded((n) => n + 1); // trigger re-render → draw effect fires
        };
    }, [userImage]);
    ...
    }, [draw, glassesLoaded, userImageLoaded]);
